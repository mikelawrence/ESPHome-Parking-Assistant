...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        # cv.Optional(CONF_DIR_SAMPLE_NUM, default=10): cv.int_range(min=2, max=1000),
...
def final_validate(config):
    if config[CONF_MODEL] == MODEL_TF_LUNA:
        if config[CONF_SAMPLE_RATE] > 250:
            raise cv.Invalid("Model" + MODEL_TF_LUNA + " Sample Rate must be at most 250.")
